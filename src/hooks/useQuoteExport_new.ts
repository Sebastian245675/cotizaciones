...
export const useQuoteExport = () => {
    ...
    try {
      ...
      clientCards.forEach((card) => {
        ...
        card.data.forEach(([label, value], index) => {
          // Espaciado ligeramente mayor para el email (índice 1)
          const spacing = index === 1 ? 17 : 15; // Email con 2 puntos extra de espaciado
          const contentY = yPosition + 32 + (index === 0 ? 0 : index === 1 ? 15 : 32); // 0, 15, 32
          doc.setFont('helvetica', 'bold');
          doc.text(label, clientCardX + 8, contentY);
          doc.setFont('helvetica', 'normal');
          // Ajustar el value ligeramente hacia la derecha para evitar superposición
          const valueText = doc.splitTextToSize(value, clientCardWidth - 60);
          doc.text(valueText, clientCardX + 45, contentY + 8);
        });
  ...
  const exportToWord = async (
    ...
    try {
        ...
        sections: [
          {
            ...
            children: [
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Cotización #: MIRG-${submission.id.substring(0, 8)}`,
                    size: 22,
                  }),
                ],
              }),
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Nombre: ${clientInfo["1"] || clientInfo.nombreCompleto || 'N/A'}`,
                    size: 22,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Email: ${clientInfo.email || submission.customerEmail || 'N/A'}`,
                    size: 22,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Teléfono: ${clientInfo.telefono || submission.customerPhone || 'N/A'}`,
                    size: 22,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Empresa: ${clientInfo.empresa || 'N/A'}`,
                    size: 22,
                  }),
                ],
              }),
              ...(clientInfo.direccion ? [
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `Dirección: ${clientInfo.direccion}`,
                      size: 22,
                    }),
                  ],
                })
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: `Tipo de servicio: ${clientInfo.tipoDeServicio || 'No especificado'}`,
                    size: 22,
                  }),
                ],
              }),
              ...(clientInfo.descripcionDelProyecto ? [
                new Paragraph({ children: [] }),
                new Paragraph({
                  children: [
                    new TextRun({
                ...
                new Paragraph({
                  children: [
                    new TextRun({
                      text: clientInfo.descripcionDelProyecto,
                      size: 20,
                      italics: true,
                    }),
                  ],
              ...
              new Paragraph({
                children: [
                  new TextRun({
                    text: "• Esta propuesta tiene validez de 30 días calendario",
                    size: 20,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: "• Se requiere anticipo del 50% para iniciar el proyecto",
                    size: 20,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: "• Tiempo estimado de desarrollo: 30-45 días hábiles",
                    size: 20,
                  }),
                ],
              }),
              new Paragraph({
                children: [
                  new TextRun({
                    text: "• Garantía de 90 días por defectos de funcionamiento",
                    size: 20,
                  }),
                ],
              }),
      ...
      const filename = `MIRG_Propuesta_${clientInfo.tipoDeServicio || 'General'}_${submission.id.substring(0, 8)}.docx`;
