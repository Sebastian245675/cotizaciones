...
export const useQuoteExport = () => {
    ...
    try {
      ...
      if (productsWithImages.length > 0 && productsWithImages.some(p => p.imageUrl)) {
        ...
        productsWithImages.forEach((product, index) => {
          ...
          if (product.description && product.description.trim()) {
            ...
            const descriptionText = product.description.length > 45 ? 
              product.description.substring(0, 45) + '...' : product.description;
            doc.text(descriptionText, contentStartX, nameY + 8);
          }
          ...
          // Total (en negrita)
          doc.setFont('helvetica', 'bold');
          doc.text(product.total || 'A cotizar', pageWidth - margin - 5, centerY, { align: 'right' });
        ...
        try {
          (doc as any).autoTable({
            ...
            body: productData.map(row => [row[0], row[1], row[3]]), // Solo nombre, cantidad y total
            theme: 'grid',
            headStyles: {
              fillColor: [15, 23, 42],
      ...
      if (!tableSuccessful) {
        ...
        productData.forEach((row, index) => {
          ...
          doc.text(row[3], pageWidth - margin - 3, yPosition + 9, { align: 'right' }); // Total (saltamos precio unitario)
  ...
  const exportToPDFAndEmail = async (
    ...
    try {
      // === ESTRATEGIA SIMPLE: REUTILIZAR exportToPDF ===
      // Crear una versión modificada que no descarga sino que retorna el PDF
      
      // Primero, generar el PDF usando exactamente la misma lógica de exportToPDF
      // pero capturándolo en lugar de descargarlo
      
      // Llamar a exportToPDF pero interceptar la descarga
      const originalSave = (jsPDF.prototype as any).save;
      let pdfBlob: Blob | null = null;
      
      // Interceptar temporalmente el método save
      (jsPDF.prototype as any).save = function(filename?: string) {
        const pdfData = this.output('blob');
        pdfBlob = pdfData;
        // No ejecutar la descarga original
      };
      
      try {
        // Ejecutar exportToPDF - esto generará exactamente el mismo PDF
        await exportToPDF(submission, companyInfo);
      } finally {
        // Restaurar el método save original
        (jsPDF.prototype as any).save = originalSave;
      }
      ...
      // === TABLA DE PRODUCTOS CON SOPORTE PARA IMÁGENES ===
      ...
      if (products.length > 0) {
        ...
        doc.text('CANT.', currentX + 8, tableStartY + 8, { align: 'center' });
        currentX += colWidths.quantity;
        doc.text('TOTAL', currentX + 15, tableStartY + 8, { align: 'center' });

        yPosition = tableStartY + headerHeight;

        let totalGeneral = 0;
        products.forEach((product, index) => {
          let rowHeight = 20; // Altura base aumentada para imágenes
          
          // Si hay imagen, calcular altura necesaria
          if (product.imageUrl && product.imageUrl.trim()) {
            rowHeight = Math.max(rowHeight, 25); // Altura mínima para imágenes
          }
          ...
          if (yPosition + rowHeight > pageHeight - 50) {
            ...
            currentX = margin + 5;
            doc.text('DESCRIPCIÓN', currentX, yPosition + 8);
            currentX += colWidths.description;
            doc.text('CANT.', currentX + 8, yPosition + 8, { align: 'center' });
            currentX += colWidths.quantity;
            doc.text('TOTAL', currentX + 15, yPosition + 8, { align: 'center' });
          ...
          // Si hay imagen, agregarla primero y ajustar posición del texto
          if (product.imageUrl && product.imageUrl.trim()) {
            try {
              const imageX = currentX;
              const imageY = yPosition + 2;
              const maxImageWidth = 18;
              ...
              if (product.imageUrl.startsWith('data:image/') || 
                ...
                try {
                  doc.addImage(product.imageUrl, 'JPEG', imageX, imageY, maxImageWidth, maxImageHeight);
                  textStartX = imageX + maxImageWidth + 3; // Mover texto después de la imagen
                } catch (imageError) {
                  console.log('Error al cargar imagen del producto:', imageError);
                }
              }
            } catch (error) {
          ...
          const availableWidth = colWidths.description - (textStartX - currentX) - 5;
          const descLines = doc.splitTextToSize(description, availableWidth);
          ...
          // Si hay descripción adicional, mostrarla
          if (product.description && product.description.trim()) {
            doc.setFontSize(7);
            doc.setTextColor(100, 116, 139);
            const additionalDesc = doc.splitTextToSize(product.description, availableWidth);
            if (additionalDesc.length > 0) {
              doc.text(additionalDesc[0], textStartX, descY);
            }
            doc.setFontSize(9);
            doc.setTextColor(31, 41, 55);
          }
