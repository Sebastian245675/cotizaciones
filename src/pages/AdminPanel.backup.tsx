...
export const AdminPanel: React.FC = () => {
  ...
  // Nuevos estados para el manejo de modos
  ...
  // Cargar modo guardado al inicializar
  useEffect(() => {
    const savedMode = localStorage.getItem('adminPanelMode') as 'ecommerce' | 'pos' | 'hybrid';
    if (savedMode) {
  ...
  const handleModeSelect = (mode: 'ecommerce' | 'pos' | 'hybrid') => {
    ...
    toast({
      ...
      description: `Panel configurado en modo ${mode === 'ecommerce' ? 'E-commerce' : mode === 'pos' ? 'POS' : 'Híbrido'}`,
    });
  };
  ...
  return (
    <div>
      {/* Si se muestra la selección de modo */}
      {showModeSelection && (
        <ModeSelection
      ...
      {/* Si se muestra el sistema POS */}
      {showPOSSystem && (
        <POSSystem onBack={closePOSSystem} />
      )}
      
      {/* Panel principal cuando no se muestra la selección de modo ni el POS */}
      {!showModeSelection && !showPOSSystem && (
              ...
              <TabsContent value="dashboard" className="space-y-6">
                ...
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-2xl font-bold text-gray-800">Modo de Operación</h2>
                    <Badge variant="outline" className="text-lg px-3 py-1">
                      {currentMode === 'ecommerce' && '🌐 E-commerce'}
                      {currentMode === 'pos' && '🏪 Punto de Venta'}
                      {currentMode === 'hybrid' && '💼 Híbrido'}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <Card 
                      className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${
                        currentMode === 'ecommerce' ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'
                      }`}
                      onClick={() => setCurrentMode('ecommerce')}
                    >
                      <CardContent className="p-6 text-center">
                        <Globe className={`mx-auto mb-4 ${currentMode === 'ecommerce' ? 'text-blue-500' : 'text-gray-400'}`} size={48} />
                        <h3 className="font-semibold text-lg mb-2">Modo E-commerce</h3>
                        <p className="text-sm text-gray-600 mb-4">Gestión completa de tienda online</p>
                    ...
                    <Card 
                      className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${
                        currentMode === 'pos' ? 'ring-2 ring-green-500 bg-green-50' : 'hover:bg-gray-50'
                      }`}
                      onClick={() => setCurrentMode('pos')}
                    >
                      <CardContent className="p-6 text-center">
                        <Store className={`mx-auto mb-4 ${currentMode === 'pos' ? 'text-green-500' : 'text-gray-400'}`} size={48} />
                        <h3 className="font-semibold text-lg mb-2">Modo POS</h3>
                        <p className="text-sm text-gray-600 mb-4">Sistema punto de venta completo</p>
                        {currentMode === 'pos' && (
                    ...
                    <Card 
                      className={`cursor-pointer transition-all duration-300 hover:shadow-lg ${
                        currentMode === 'hybrid' ? 'ring-2 ring-purple-500 bg-purple-50' : 'hover:bg-gray-50'
                      }`}
                      onClick={() => setCurrentMode('hybrid')}
                    >
                      <CardContent className="p-6 text-center">
                        <Monitor className={`mx-auto mb-4 ${currentMode === 'hybrid' ? 'text-purple-500' : 'text-gray-400'}`} size={48} />
                        <h3 className="font-semibold text-lg mb-2">Modo Híbrido</h3>
                        <p className="text-sm text-gray-600 mb-4">Todas las funcionalidades</p>
                        {currentMode === 'hybrid' && (
                          <div className="space-y-2">
