...
import { 
  ...
  MapPin, Award, Zap, RefreshCw, ExternalLink, ChevronRight, Activity, FileText
} from 'lucide-react';
...
export function POSClientsManager({ isVisible }: POSClientsManagerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentClient) {
        ...
        const clientId = await createPOSClient(formData);
        toast.success(`Cliente ${formData.nombre} agregado exitosamente con ID único generado`, {
          icon: '🎉',
          style: {
            background: 'linear-gradient(45deg, #10b981, #34d399)',
            color: 'white',
  ...
  const filteredClients = clients.filter(client => {
    ...
    const matchesSearch = client.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      (client.clienteId && client.clienteId.includes(searchTerm)) ||
    ...
    const matchesOrigen = origenFilter === 'todos' || 
      (origenFilter === 'manual' && (!client.origen || client.origen === 'manual')) ||
      (origenFilter === 'cotizacion' && client.origen === 'cotizacion') ||
      (origenFilter === 'venta' && client.origen === 'venta');
  ...
  const handleRegisterSale = async (client: POSClient) => {
    const clientId = client.clienteId || client.id || 'cliente';
    const amountStr = prompt(`Registrar venta para ${client.nombre} (ID: ${clientId}). Ingrese el monto:`);
    if (!amountStr) return;
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200/50 mb-8 p-6">
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <select
                  value={origenFilter}
                  onChange={(e) => setOrigenFilter(e.target.value as 'todos' | 'manual' | 'cotizacion' | 'venta')}
                  className="px-4 py-3 bg-white/90 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 shadow-sm"
                >
                  <option value="todos">Todos los clientes</option>
                  <option value="manual">Creados manualmente</option>
                  <option value="cotizacion">Desde cotizaciones</option>
                  <option value="venta">Desde ventas</option>
                </select>
              </div>
        ...
        {(viewMode === 'add' || viewMode === 'edit') && (
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-200/50 p-8">
            <div className="mb-8">
              ...
              {currentClient && (
                <p className="text-slate-600">ID Cliente: <span className="font-mono font-semibold">{currentClient.clienteId || 'Sin ID asignado'}</span></p>
              )}
            </div>
                  ...
                  {sortedClients.map((client) => (
                    <tr key={client.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold">
                            {client.clienteId ? client.clienteId.slice(-2) : 'ID'}
                          </div>
                          <span className="font-mono font-semibold text-slate-900">{client.clienteId || 'Sin ID'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                      ...
                      <td className="px-6 py-4 text-slate-600">
                        {client.fechaRegistro?.toDate?.() ? client.fechaRegistro.toDate().toLocaleDateString() : 'N/A'}
                      </td>
                      <td className="px-6 py-4">
            ...
            {sortedClients.map((client) => (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-3 py-1 rounded-lg text-sm font-bold">
                      ID: {client.clienteId || 'Sin ID'}
                    </div>
                    {client.origen === 'cotizacion' && (
                      <div className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-2 py-1 rounded-md text-xs font-bold flex items-center gap-1">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  {client.origen === 'cotizacion' && client.codigoCotizacion && (
                    <div className="flex items-center gap-2 text-emerald-600">
                      <FileText className="w-4 h-4" />
                      <span className="text-xs font-semibold">
                        Desde cotización: {client.codigoCotizacion}
                      </span>
                    </div>
                  )}
