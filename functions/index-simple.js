  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border-radius: 10px; border: 1px solid #e0e0e0; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
      <div style="text-align: center; margin-bottom: 20px;">
        <h1 style="color: #3b82f6;">REGALA ALGO</h1>
      </div>
      ...
      <p style="font-size: 16px; line-height: 1.6; color: #333;">Le enviamos la cotización solicitada:</p>
      ...
      <div style="margin-top: 30px; padding: 20px; background-color: #f0f9ff; border-radius: 5px; border-left: 4px solid #3b82f6;">
        <p style="margin: 0; color: #333;"><strong>📍 Dirección:</strong> Tu dirección aquí</p>
        <p style="margin: 5px 0 0 0; color: #333;"><strong>📞 Teléfono:</strong> Tu teléfono aquí</p>
        <p style="margin: 5px 0 0 0; color: #333;"><strong>📧 Email:</strong> tu-email@ejemplo.com</p>
      </div>
      
      <p style="font-size: 14px; color: #666; text-align: center; margin-top: 30px;">
        Gracias por confiar en nosotros.<br>
        <strong>REGALA ALGO</strong>
      </p>
    </div>
  `;
};
...
exports.sendQuotePDF = functions.https.onCall(async (data, context) => {
  try {
    ...
    const htmlContent = createQuoteEmailHTML(quoteData);
    ...
    const mailOptions = {
      ...
      to: data.email,
      subject: `Cotización ${quoteData.quoteNumber} - REGALA ALGO`,
      html: htmlContent,
      attachments: [
        {
